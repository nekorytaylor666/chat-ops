...
import {
  useAddStageAttribute,
  useUpdateStageAttribute,
...
interface StageAttributeModalProps {
  open: boolean;
...
export function StageAttributeModal({
  open,
  onOpenChange,
  stageId,
  attribute,
  io = "output",
}: StageAttributeModalProps) {
  const trpc = useTRPC();
  const { organizationId } = useOrganizationContext();
  const addAttribute = useAddStageAttribute();
  const updateAttribute = useUpdateStageAttribute();
  ...
  const [options, setOptions] = React.useState<string[]>(
    (attribute?.config as AttributeConfig)?.options?.map((o) => o.label) ?? [""]
  );
  const [targetEntityId, setTargetEntityId] = React.useState(
    (attribute?.config as AttributeConfig)?.targetEntityId ?? ""
  );
  ...
  React.useEffect(() => {
    if (open) {
      ...
      setOptions(
        (attribute?.config as AttributeConfig)?.options?.map(
          (o) => o.label
        ) ?? [""]
      );
      setTargetEntityId(
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const targetEntity = entities.find((ent) => ent.id === targetEntityId);
  ...
  const ioLabel = io === "input" ? "входных данных" : "выходных данных";
  ...
  return (
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>
            {isEditing ? "Редактировать атрибут" : `Создать атрибут ${ioLabel}`}
          </DialogTitle>
        </DialogHeader>
